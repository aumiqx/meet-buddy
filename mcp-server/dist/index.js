...
const MEETINGS_DIR = process.env.MEET_BUDDY_DIR ||
...
function readMeetingFile(meetingId, filename) {
    const filePath = path.join(MEETINGS_DIR, meetingId, filename);
    if (!fs.existsSync(filePath))
        return null;
    return fs.readFileSync(filePath, "utf-8");
}
function readMeetingMeta(meetingId) {
...
function getScreenshotList(meetingId) {
...
// ── Tools ──────────────────────────────────────────────────
server.tool("meeting_list", "List all recorded meetings", {}, async () => {
    const meetings = listMeetings();
    ...
    const details = meetings.map((id) => {
        ...
        const start = meta?.startTime || "unknown";
        const end = meta?.endTime || "ongoing";
        const words = meta?.totalWords || 0;
        const screenshots = meta?.totalScreenshots || 0;
...
server.tool("meeting_active", "Check if a meeting is currently active (has no endTime in meta)", {}, async () => {
    const meetings = listMeetings();
...
server.tool("meeting_transcript", "Get the full transcript of a meeting, or the latest N lines", {
    meeting_id: z
        .string()
        .optional()
...
server.tool("meeting_screenshots", "List screenshots from a meeting, optionally read a specific one", {
...
server.tool("meeting_meta", "Get meeting metadata (title, start/end time, stats)", {
    meeting_id: z.string().optional().describe("Meeting folder ID. Omit for most recent."),
}, async ({ meeting_id }) => {
    const id = meeting_id || getMostRecentMeeting();
    if (!id) {
...
server.tool("meeting_notes", "Read or write notes for a meeting (your analysis, action items, etc.)", {
    meeting_id: z.string().optional().describe("Meeting folder ID. Omit for most recent."),
...
server.tool("meeting_sync", "Pull latest meeting data from GitHub repo to local directory", {
    ...
    try {
        ...
        if (!fs.existsSync(tempDir)) {
            execSync(`git clone --depth 1 --filter=blob:none --sparse https://github.com/${repo}.git "${tempDir}"`, { stdio: "pipe" });
        ...
        if (fs.existsSync(meetingsSource)) {
            const meetings = meeting_id
                ? [meeting_id]
                : fs.readdirSync(meetingsSource).filter((d) => fs.statSync(path.join(meetingsSource, d)).isDirectory());
